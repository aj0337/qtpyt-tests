...
def run(outputfile):
    ...
    for e, energy in enumerate(gd.energies):
        T_total[e] = gf.get_transmission(energy)
    ...
    if comm.rank == 0:
        ...
        plt.figure()
        plt.plot(energies, T_total.real, label="Total")
        plt.yscale("log")
        plt.xlim(-3.0, 3.0)
        plt.ylim(1e-5, 1)
        ...
        plt.tight_layout()
        plt.savefig(f"{output_folder}/ET_lo_dft.png", dpi=300)
        plt.close()


pl_path = Path("./dft/leads/")
cc_path = Path("./dft/device/")
output_folder = "./output/lowdin/dft"
...
outputfile = f"{output_folder}/ET.npy"
run(outputfile)
