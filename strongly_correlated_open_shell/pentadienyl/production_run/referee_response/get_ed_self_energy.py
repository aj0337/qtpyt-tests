...
V = np.loadtxt(f"{input_folder}/U_matrix_ppp_2.0.txt")
...
DC = np.load(f"{output_folder}/ed_dcc_diag_Uppp_2.0.npy")
DC = np.diag(DC)
...
np.save(f"{output_folder}/ed_sigma_ppp_2.0.npy", sigma_ret)
