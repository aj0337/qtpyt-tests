...
basis = {
        'Au':num_spherical_harmonics_Au,
        'H':num_spherical_harmonics_H,
        'C':num_spherical_harmonics_C,
        'S':num_spherical_harmonics_S,
        }

atoms_pl = read(pl_path/'leads.xyz')
basis_pl = Basis.from_dictionary(atoms_pl, basis)

atoms_cc = read(cc_path/'device.xyz')
...
h_pl_ii, s_pl_ii, h_pl_ij, s_pl_ij = map(lambda m: m[0], prepare_leads_matrices(h_pl_k, s_pl_k, (20,5,2), align=(0,h_cc_k[0,0,0]))[1:])
