...
for U_val in U_list:
    ed_data_folder = f"{base_ed_data_folder}/Uppp_{U_val:.1f}"
    if rank == 0:
    ...
    def run(outputfile: str):
        ...
        if comm.rank == 0:
            ...
            plt.tight_layout()
            plt.savefig(f"{ed_data_folder}/ET_Uppp_{U_val:.1f}.png", dpi=300)
            plt.close()

    # -----------------------------
    # Load ED self-energy for this U
    # -----------------------------
    ed_self_energy_file = f"{ed_data_folder}/ed_sigma_ppp_{U_val:.1f}.npy"
    if rank == 0 and (not os.path.isfile(ed_self_energy_file)):
        raise FileNotFoundError(
            f"Missing ED self-energy for U={U_val:.1f} eV:\n  {ed_self_energy_file}"
        )
    ...
    outputfile = f"{ed_data_folder}/ET_Uppp_{U_val:.1f}.npy"
    run(outputfile)

    gf.selfenergies.pop()

    if rank == 0:
        print(f"[Done] U={U_val:.1f} saved:")
        print(f"  {outputfile}")
        print(f"  {ed_data_folder}/ET_Uppp_{U_val:.1f}.png")
