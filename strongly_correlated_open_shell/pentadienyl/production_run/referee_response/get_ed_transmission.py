...
def run(outputfile):
    ...
    if comm.rank == 0:
        ...
        plt.tight_layout()
        plt.savefig(f"{ed_data_folder}/ET_Uppp_2.0.png", dpi=300)
        plt.close()


data_folder = "../output/lowdin"
ed_data_folder = "../output/lowdin/ed/referee_response"
index_active_region = np.load(f"{data_folder}/index_active_region.npy")
self_energy = np.load(f"{data_folder}/self_energy.npy", allow_pickle=True)
ed_self_energy_file = f"{ed_data_folder}/ed_sigma_ppp_2.0.npy"
...
outputfile = f"{ed_data_folder}/ET_Uppp_2.0.npy"
run(outputfile)
gf.selfenergies.pop()
