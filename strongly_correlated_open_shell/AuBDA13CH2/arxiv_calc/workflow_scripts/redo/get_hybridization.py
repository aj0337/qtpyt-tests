...
de = 0.01
energies = np.arange(-10.0, 10.00 + de / 2.0, de).round(7)
beta = 1000
mu = 0.0
...
# Matsubara why are we setting eta to 0.0?
gf.eta = 0.0
...
if comm.rank == 0:
    ...
    np.save(os.path.join(data_folder, "energies.npy"), energies + 1.0j * eta)
    # Effective Hamiltonian
    np.save(os.path.join(data_folder, "hamiltonian.npy"), hyb.H)
    np.save(
        os.path.join(data_folder, "occupancies.npy"),
