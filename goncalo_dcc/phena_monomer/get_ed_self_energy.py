...
for U_val in U_list:
    ...
    dc_diag_path = f"{output_folder}/ed_dc_diag_Uppp_{U_val:.1f}.npy"
    if not os.path.isfile(dc_diag_path):
        # fallback to your older naming if present
        old_path = f"{output_folder}/ed_dcc_diag_Uppp_{U_val:.1f}.npy"
        if os.path.isfile(old_path):
            dc_diag_path = old_path
        else:
            raise FileNotFoundError(
                f"DC diagonal file not found for U = {U_val:.1f} eV.\n"
                f"Tried:\n  {output_folder}/ed_dc_diag_Uppp_{U_val:.1f}.npy\n"
                f"  {output_folder}/ed_dcc_diag_Uppp_{U_val:.1f}.npy"
            )
    ...
    out_sigma = f"{output_folder}/ed_sigma_ppp_{U_val:.1f}.npy"
