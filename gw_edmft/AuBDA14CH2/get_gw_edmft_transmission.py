...
def run(outputfile):
    ...
    if comm.rank == 0:
        ...
        plt.plot(energies, T.real, label="GW+eDMFT")
        plt.yscale("log")
...
edmft_data_folder = "./output/lowdin/gw_edmft"
index_active_region = np.load(f"{data_folder}/index_active_region.npy")
self_energy = np.load(f"{data_folder}/self_energy.npy", allow_pickle=True)
edmft_sigma_file = f"{edmft_data_folder}/self_energy_no_dcc.npy"
energies = np.load(f"{edmft_data_folder}/energies.npy")
sigma_edmft = np.load(f"{edmft_sigma_file}")
...
if comm.rank == 0:
    edmft_sigma = load(edmft_sigma_file)
else:
    edmft_sigma = None
...
edmft_sigma = comm.bcast(edmft_sigma, root=0)
self_energy[2] = edmft_sigma
gf.selfenergies.append((imb, self_energy[2]))

outputfile = f"{edmft_data_folder}/ET_no_dcc.npy"
run(outputfile)
gf.selfenergies.pop()
