...
# rearrange Hamiltonian according to symbols: Au, H, N, C
xbfs = []
atoms_sorted = Atoms()
for symbol in ['Au', 'H', 'N', 'C']:
    ibfs, ia = extract_orbitals(atoms, [symbol], basis)
    xbfs.extend(ibfs)
    atoms_sorted += ia

# debug
#for iatom,jatom in zip(atoms,atoms_sorted):
#    print(iatom.symbol, jatom.symbol)

# replace atomic sequence
atoms = atoms_sorted

h_sort = np.zeros((nbfs,nbfs), dtype='complex')
s_sort = np.zeros((nbfs,nbfs), dtype='complex')
for i,ib in enumerate(xbfs):
    for j,jb in enumerate(xbfs):
        h_sort[i,j] = h_sub[ib,jb]
        s_sort[i,j] = s_sub[ib,jb]

# save sorted (H,S) to HDD
pickle.dump((h_sort.astype(complex), s_sort.astype(complex)), open('scatt_hs_sort.pckl', 'wb'), 2)
...
# edit subdiagonalized AND sorted (H,S) perform cut-coupling of C non-pz bfs
h_cut = np.copy(h_sort)
s_cut = np.copy(s_sort)
...
# extracted with algorithm and threshold = 0.1
#cpzd = [636, 638, 639, 641, 642, 645, 646, 647, 661, 664, 666, 668, 670, 674, 677, 681, 683, 687, 690, 694, 696, 700, 703, 704, 707, 708, 709, 718, 721, 725, 727]
...
for i in cpz:
    for j in p:
        if abs(h_sort[i,j].real) > threshold:
            ibfs.append(j)
            print(i,j,abs(h_sort[i,j].real))
...
axes[0,1].title.set_text('subdiagonalized (sorted)')
axes[1,2].title.set_text('C-pz (sorted)')
axes[1,3].title.set_text('C-pz+d (sorted)')
...
axes[0,1].spy(h_sort,               precision = precision)
...
axes[1,1].spy(h_sort[-nm:,-nm:],    precision = precision)
axes[1,2].spy(h_pz[-nm:,-nm:],      precision = precision)
axes[1,3].spy(h_pzd[-nm:,-nm:],     precision = precision)
