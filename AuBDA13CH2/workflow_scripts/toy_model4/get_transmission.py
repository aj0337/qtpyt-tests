...
energies_folder = "../../output/compute_run/toy_model4"
output_folder = f"../../output/compute_run/toy_model4/nbaths_{nbath}_U_{U}_{'with_dc_correction' if double_counting else 'without_dc_correction'}_{'adjust_mu' if adjust_mu else 'no_adjust_mu'}"
os.makedirs(output_folder, exist_ok=True)

index_active_region = np.load(f"{data_folder}/index_active_region.npy")
self_energy = np.load(f"{data_folder}/self_energy.npy", allow_pickle=True)
dmft_sigma_file = f"{output_folder}/dmft_sigma.npy"
energies = np.load(f"{energies_folder}/energies.npy")
