...
los_path = Path('../../localized_orbitals/bda_active/')

h_pl_k, s_pl_k = np.load(pl_path / 'hs_pl_k.npy')
h_cc_k, s_cc_k = map(lambda m: m.astype(complex),
                     np.load(los_path / f'hs_lolw_no_sign_flip_no_lowdin_k.npy'))
...
for eta in etas:
    ...
    if comm.rank == 0:
        np.save(f'no_sign_flip_no_lowdin/ET_eta{eta}.npy', (energies, T))
