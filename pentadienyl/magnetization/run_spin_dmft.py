...
for beta in betas:
    ...
    staggered_spin = np.sum(
        (-1) ** np.arange(len(spin_occps)) * (spin_occps[:, 0] - spin_occps[:, 1]) / 2
    )

    staggered_spin_data.append((beta, staggered_spin))

    print(f"Staggered spin at beta = {beta}: {staggered_spin:.6f}")

    if abs(staggered_spin) < 1e-2:
        consecutive_low_spin_count += 1
        if consecutive_low_spin_count == 2:
            print(
                f"Staggered spin < 1e-2 for two consecutive betas: "
                f"{betas[betas.index(beta) - 1]}, {beta}. Breaking loop."
            )
            break
    else:
        consecutive_low_spin_count = 0

np.save(f"{data_folder}/staggered_spin_vs_beta.npy", staggered_spin_data)
print("Saved staggered spin vs beta data for analysis.")
