...
import numpy as np
from edpyt.dmft import DMFT, Converged, FailedToConverge, Gfimp, dmft_step
...
output_folder = "../../output/compute_run/reference_branch_conv"
...
hyb_mats = np.fromfile(f"{data_folder}/matsubara_hybridization.bin", complex).reshape(
    z_mats.size, len_active, len_active
)
...
try:
    root(distance, delta_prev, method="broyden1", options={"jac_options": {"alpha": 0.1}})
...
np.save('opt_delta_dmft', delta_prev)
np.save('opt_mu_dmft', gfloc.mu)
